...
fn problem_2 (mut cf: usize, mut arr: [i64; 100]) {
    ...
    /*
        Task: Find the largest palindrom made from the product of two 3-digit numbers.
        Example 2 digit number: 9009 (91 * 99)

        Breakdown:
        our original number is 12345, reverse its digits = 54321
        now add reversed number to original number = 66666

        for us, this number is palindrom but if it isn't..
        use the sum of the original and reversed number and repeat the process until we get a palindrom
    */

// solution:
const o: [i64; 5] = [1, 2, 3, 4, 5];

fn problem_3 (num: [i64; 5]) {
    
    // empty arrays
    let mut reversed: [i64; 5] = [0, 0, 0, 0, 0];
    let mut answer: [i64; 5] = [0, 0, 0, 0, 0];

    // invert number
    for x in o {
        reversed[x as usize] = o[num.len() - x as usize];
    }

    // sum the two numbers
    for x in reversed {
        answer[x as usize] = o[x as usize] + reversed[0];
    }

    // check if is a palindrom
    if (answer[4] != answer[0]) {
        ...
        problem_3(num); // change to counter so that we can iterate through all possibilities
    } 
